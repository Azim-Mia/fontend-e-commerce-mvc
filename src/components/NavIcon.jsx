...
const NavIcon =()=>{
  ...
  const [count,setCartLength]=useState(0);
...
const handleProfile =()=>{
  ...
  <div className="flex index-full gap-4 xs:text-sm sm:gap-6 lg:gap-10">
   ...
   <div className="relative" onClick={()=>serIsCartOpen((prev)=>!prev)}>
    ...
    <div className="absolute -top-4 -right-4 w-6 h-6 xs:h-4 xs:w-4 bg-lama rounded-full text-sm text-white flex justify-center items-center">{count}</div>
   </div>
{isCartOpen && <CartModuls onRemoveCart={removeCart} onCartLength={setCartLength}/>}
